                       ...
                       const std::string rawData, const std::string algo)
{
    ...
    if (algo == "SHA256")
    {
        ...
        if (!PKCS5_PBKDF2_HMAC(
                ...
                seed.size(), iter, EVP_sha256(), hashLen, output.data()))
        {
    ...
    if (algo == "SHA384")
    {
        ...
        if (!PKCS5_PBKDF2_HMAC(
                ...
                rawData.length() + 1,
                reinterpret_cast<const unsigned char*>(seed.data()),
                seed.size(), iter, EVP_sha384(), hashLen, output.data()))
        {
            phosphor::logging::log<phosphor::logging::level::ERR>(
    ...
    if (fs::exists(seedFile.c_str()))
    {
        ...
        if (hashAlgo == "SHA256")
        {
            ...
            if (!PKCS5_PBKDF2_HMAC(
                    ...
                    reinterpret_cast<const unsigned char*>(seed.data()),
                    seed.size(), iter, EVP_sha256(), mdLen, mNewPwdHash.data()))
            {
                phosphor::logging::log<phosphor::logging::level::ERR>(
        ...
        if (hashAlgo == "SHA384")
        {
            ...
            if (!PKCS5_PBKDF2_HMAC(
                    ...
                    newPassword.length() + 1,
                    reinterpret_cast<const unsigned char*>(seed.data()),
                    seed.size(), iter, EVP_sha384(), mdLen, mNewPwdHash.data()))
            {
                phosphor::logging::log<phosphor::logging::level::ERR>(
